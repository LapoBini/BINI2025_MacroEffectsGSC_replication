...
function readdata_haver(
    ...
    scale_irf = legend[incl,3]
    prior     = legend[incl,end-1] |> Array{Float64,1};
    transf    = legend[:, 4:6] |> Array{Bool,2};
    ...
    instrument   = [];
    instrumented = [];
    if iv_spreads in sheetname
        iv_aux     = database[iv_spreads][:];
    ...
    return transf_data, ref_dates, tickers, prior, sign_s, name_s, 
           instrument, pos_policy, base_frq, scale_irf, transf[incl,:]
